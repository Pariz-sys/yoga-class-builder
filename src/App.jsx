...
function saveClasses(classes) {
    try {
        ...
        // localStorage full (can happen with many large base64 images)
    ...
    return (
        <div
            ...
            onTouchStart={(e) => {
                if (!boardMode) return;
                setPressing(true);
                pressTimer.current = setTimeout(() => onDragStart(e, index, "touch"), 300);
            }}
            onTouchEnd={() => { clearTimeout(pressTimer.current); setPressing(false); }}
            onMouseDown={(e) => { if (boardMode) onDragStart(e, index, "mouse"); }}
        >
            <div style={{
...
function AddPoseModal({ onClose, onAdd }) {
    ...
    return (
        <Sheet onClose={onClose}>
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Warrior I"
            ...
            <div style={{ display: "flex", gap: "8px", marginBottom: "24px" }}>
                {[15, 30, 45, 60].map(s => (
                    <button key={s} onClick={() => setDuration(String(s))} style={{
            ...
            <PrimaryBtn disabled={!name.trim()} onClick={() => { if (name.trim()) { onAdd({ name: name.trim(), duration: parseInt(duration) || 30, imageUrl }); onClose(); } }}>
                Add to Class
            </PrimaryBtn>
        </Sheet>
    );
    ...
    return (
        <Sheet onClose={onClose}>
            ...
            <p style={{ margin: "0 0 20px", fontSize: "14px", color: "#718096", fontFamily: FONT }}>
                Swapping "{poses[targetIndex]?.name}" — tap a pose to swap with
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", maxHeight: "260px", overflowY: "auto", paddingBottom: "4px" }}>
                {poses.map((pose, i) => i === targetIndex ? null : (
                    <div key={i} onClick={() => { onSwap(targetIndex, i); onClose(); }} style={{ cursor: "pointer" }}>
                        <PoseCard pose={pose} index={i} onDelete={() => { }} onDragStart={() => { }} boardMode={false} />
                    </div>
                ))}
            </div>
...
function PlayerView({ poses, onClose }) {
    ...
    const [timeLeft, setTimeLeft] = useState(poses[0]?.duration || 30);
    const [paused, setPaused] = useState(false);

    useEffect(() => {
        if (paused) return;
        if (timeLeft <= 0) {
            if (current < poses.length - 1) { const n = current + 1; setCurrent(n); setTimeLeft(poses[n].duration); }
            return;
        }
    ...
    const progress = (pose.duration - timeLeft) / pose.duration;
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                background: pose.imageUrl ? "transparent" : PLACEHOLDER_COLORS[current % PLACEHOLDER_COLORS.length],
            ...
            <h2 style={{ color: "#fff", fontSize: "28px", fontWeight: "700", margin: "0 0 8px" }}>{pose.name}</h2>
            ...
            <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
                <NavBtn disabled={current === 0} onClick={() => { if (current > 0) { const n = current - 1; setCurrent(n); setTimeLeft(poses[n].duration); } }}>‹</NavBtn>
                ...
                <NavBtn disabled={current === poses.length - 1} onClick={() => { if (current < poses.length - 1) { const n = current + 1; setCurrent(n); setTimeLeft(poses[n].duration); } }}>›</NavBtn>
            </div>
        ...
        <div style={{ minHeight: "100vh", background: BG, fontFamily: FONT, maxWidth: "480px", margin: "0 auto" }}>
            ...
            <div style={{ padding: "20px 20px 120px" }}>
                ...
                {classes.map(cls => {
                    ...
                    const total = cls.poses.reduce((s, p) => s + p.duration, 0);
                    return (
                        <div key={cls.id} style={{ position: "relative", marginBottom: "12px" }}>
                            <div
                                onClick={() => { setMenuId(null); onOpen(cls.id); }}
                                style={{
                            ...
                            {menuId === cls.id && (
                                <div style={{
                                    ...
                                    {[
                                        { label: "✏️  Rename", action: () => { onEdit(cls.id); setMenuId(null); } },
                                        { label: "🗑️  Delete", action: () => { onDelete(cls.id); setMenuId(null); }, danger: true },
                                    ].map((item, i) => (
                                        <button key={i} onClick={item.action} style={{
                                            display: "block", width: "100%", padding: "14px 18px", border: "none",
                                            background: "none", textAlign: "left", fontSize: "15px", fontWeight: "600",
                                            color: item.danger ? "#E53E3E" : DARK,
                                            cursor: "pointer", fontFamily: FONT,
                                            borderBottom: i === 0 ? "1px solid #F0F0EE" : "none",
                                        }}>{item.label}</button>
                                    ))}
                                </div>
                            )}
...
function ClassEditor({ cls, onBack, onUpdate }) {
    ...
    const handleDragStart = useCallback((e, index) => {
        ...
        const onMove = (ev) => {
            ...
            const card = el?.closest("[data-pose-index]");
        ...
        const onUp = () => {
            ...
            setDragOverIndex(over => {
                if (over !== null && dragData.current.startIndex !== null && over !== dragData.current.startIndex) {
                    setPoses(prev => {
                        const arr = [...prev];
                        const [moved] = arr.splice(dragData.current.startIndex, 1);
    ...
    const totalDuration = poses.reduce((s, p) => s + p.duration, 0);
        ...
        <div style={{ minHeight: "100vh", background: BG, fontFamily: FONT, maxWidth: "480px", margin: "0 auto" }}>
            ...
            <div style={{ background: CARD_BG, padding: "16px 24px 0", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                ...
                <div style={{ display: "flex", gap: "20px", paddingBottom: "14px" }}>
                    {[
                        { val: poses.length, label: "poses" },
                        { val: formatTime(totalDuration), label: "total" },
                        { val: poses.length > 0 ? Math.round(totalDuration / poses.length) + "s" : "0s", label: "avg hold" },
                    ].map((s, i, arr) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: "20px" }}>
                            <div>
                                <div style={{ fontSize: "20px", fontWeight: "800", color: DARK }}>{s.val}</div>
                                <div style={{ fontSize: "11px", color: "#718096", fontWeight: "600" }}>{s.label}</div>
                            </div>
                            {i < arr.length - 1 && <div style={{ width: "1px", height: "30px", background: "#E2E8F0" }} />}
                        </div>
                    ))}
                </div>
            </div>
                        ...
                        {poses.map((pose, i) => (
                            <div key={i} data-pose-index={i}
                                ...
                                <PoseCard pose={pose} index={i} boardMode
                                    onDelete={idx => setPoses(prev => prev.filter((_, j) => j !== idx))}
                                    onDragStart={handleDragStart}
                                />
                                <button onClick={() => setSwapTarget(i)} style={{
                        ...
                        <div style={{ background: "#fff", borderRadius: "16px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                            {poses.map((pose, i) => (
                                <div key={i} style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: i < poses.length - 1 ? "1px solid #F0F0EE" : "none", gap: "12px" }}>
                                    ...
                                    <button onClick={() => setPoses(prev => prev.filter((_, j) => j !== i))} style={{ padding: "6px", border: "none", background: "none", color: "#CBD5E0", fontSize: "16px", cursor: "pointer" }}>✕</button>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {showAddModal && <AddPoseModal onClose={() => setShowAddModal(false)} onAdd={p => setPoses(prev => [...prev, p])} />}
            {swapTarget !== null && <SwapModal poses={poses} targetIndex={swapTarget} onSwap={(a, b) => setPoses(prev => { const arr = [...prev];[arr[a], arr[b]] = [arr[b], arr[a]]; return arr; })} onClose={() => setSwapTarget(null)} />}
            {showPlayer && <PlayerView poses={poses} onClose={() => setShowPlayer(false)} />}
        </div>
    );
}
...
function StatusBar() {
    return (
        <div style={{ height: "44px", background: CARD_BG, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", fontSize: "12px", color: "#4A5568", fontWeight: "600", borderBottom: "1px solid rgba(0,0,0,0.06)", position: "sticky", top: 0, zIndex: 10, fontFamily: FONT }}>
            <span>9:41</span>
            <div style={{ display: "flex", gap: "6px", alignItems: "center" }}><span>●●●</span><span>WiFi</span><span>🔋</span></div>
        </div>
    );
}
...
export default function App() {
    ...
    useEffect(() => {
        ...
        // Flash a tiny "Saved" toast so users know it persisted
        setSavedToast(true);
    ...
    const handleCreate = (data) => {
        ...
        setClasses(prev => [...prev, { id, poses: [], ...data }]);
    };
    ...
    const handleUpdateClass = (id, changes) => {
        ...
        <>
            {activeClass ? (
                ...
                <ClassesHome
                    ...
                    onCreate={() => setNewClassModal(true)}
            ...
            {editTarget !== null && (
                <NewClassModal
                    existing={classes.find(c => c.id === editTarget)}
